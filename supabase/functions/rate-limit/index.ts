...
function makeGetRequiredEnv(getEnv: (key: string) => string | undefined) {
  return function getRequiredEnv(name: string): string | undefined {
    const v = getEnv(name);
    return v && v.length > 0 ? v : undefined;
  };
}

export function createRateLimitHandler(deps: HandlerDeps = {}) {
  const getEnv = deps.getEnv ?? ((k) => Deno.env.get(k));
  const clientFactory = deps.createClient ?? createClient;
  const getRequiredEnv = makeGetRequiredEnv(getEnv);
  ...
  return {
    async fetch(request: Request): Promise<Response> {
      ...
      try {
        ...
        const supabaseAuth = clientFactory(supabaseUrl, supabaseAnonKey, {
          auth: {
        ...
        const supabase = clientFactory(supabaseUrl, supabaseServiceKey, {
          auth: {
            autoRefreshToken: false,
            persistSession: false,
          },
